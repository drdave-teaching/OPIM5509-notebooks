...
7. **Univariate RNN Pt 2.** Confirm `(3640, 10, 1)`; inherit `n_steps`/`n_features` from the shape; 30 red dots → 31×30+30 = 960, + 31 for the head = **991** params, spinning 10 times. Linear output, MSE, MAE tracked, early stopping on val_loss. SimpleRNN MAE **1.78** vs the window method's **1.80** — basically the same. LSTM (3,871 params) **1.76**. Then the sermon: beat **mean-only** and **persistence** (shift-1 ≈ 2.02) or you've learned nothing — and always metrics + scatter + time-series plot, so a shifted copy can't fool you.
